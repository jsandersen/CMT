...
def build():
    ...
    toxic = Toxic(clean=False)
    X, y, train_index_list, test_index_list, X_eval, y_eval = toxic.getDataSplits(n_splits=NUM_SPLITS, test_size=10000, random_state=1)
    X = np.array(X)
    y = np.array(y)
    ...
    for i in tqdm(range(5)):
        ...
        training_args = TFTrainingArguments(
            output_dir=f'./models/toxic/finetuned/Toxic_BERT_BL_{i}',          # output directory
            num_train_epochs=3,              # total number of training epochs
            per_device_train_batch_size=16,  # batch size per device during training
            per_device_eval_batch_size=64,   # batch size for evaluation
            warmup_steps=500,                # number of warmup steps for learning rate scheduler
            weight_decay=0.01,               # strength of weight decay
            logging_dir=f'./models/toxic/finetuned/Toxic_BERT_BL_{i}',            # directory for storing logs
            logging_steps=2000,
            save_total_limit = 1
        )
        ...
        model.save_pretrained(f'./models/toxic/bert/Toxic_BERT_BL_{i}', num_labels=2)
        ...
        print('predict ...')
        df.to_pickle(f"pickle/toxic/df_BERT_BL_{i}.pkl")
