...
def build():
    ...
    for i in tqdm(range(NUM_SPLITS)):
        print(f'#### run Nr. {i}')
        model = TFDistilBertForSequenceClassification.from_pretrained(f'./models/toxic/bert/Toxic_BERT_BL_{i}', num_labels=2)
        
        print('predict')
        df = bert_predict(model, test_encodings_tf, y_eval, T=50)
        
        print('predict ...')
        df.to_pickle(f"pickle/toxic/df_BERT_MCD_{i}.pkl")
