...
def build():
    ...
    for _ in range(NUM_SPLITS):
        ...
        models_as = [getCNN2(MAX_SEQUENCE_LENGTH, 2, embedding_layer) for i in range(T)]
    ...
    for i in range(NUM_SPLITS):
        for t in range(T):
            ...
            models_as_set[i][t].save(f'pickle/imdb/tmp_imdb_en_cnn2_{i}_{t}.h5') # save memory
            models_as_set[i][t].save(f'models/imdb/model_CNN2_EN_{i}_{t}')
    ...
    for i in range(NUM_SPLITS):
        model_as = []
        for t in range(T):
            m = tf.keras.models.load_model(f'pickle/imdb/tmp_imdb_en_cnn2_{i}_{t}.h5')
            model_as.append(m)
