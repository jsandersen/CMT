...
def f1_score_manuallyNoise(df, p, u=None, noise = 0, average='binary'):
    l = len(df)
    h = int(l*p)
    ...
    with SuppressPandasWarning():
        df_manuell.loc[:, 'y'] = df_manuell['rand_mod']
    df_sum = pd.concat([ df_auto, df_manuell ],ignore_index=True)
...
def plot_conf_line(array, d_mean_r, d_std_r, label, color):
    ...
    plt.plot((d_mean_r[0], d_mean_r[200]), color=color, linestyle='dashed')
    plt.fill_between(array, d_mean_r + d_std_r, d_mean_r - d_std_r, color=color, alpha=0.2)

    
    
def plot_moderation_performanceNoise(dfs, u = None, label='Random', color='black', average='binary', eps=200):
    li = []
    ...
    for df in dfs:
        ...
        for i in range_:
            i = round(i, 5)
            li_sub.append(f1_score_manuallyNoise(df, i, u, average=average))
        li.append(li_sub)
